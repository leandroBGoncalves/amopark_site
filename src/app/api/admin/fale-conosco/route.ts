...
import { listFaleConoscoMensagensAdmin } from "@/lib/fale-conosco-db";
...
export async function GET() {
  ...
  try {
    const rows = await listFaleConoscoMensagensAdmin();
    return NextResponse.json(rows);
  } catch (err) {
    console.error("GET /api/admin/fale-conosco:", err);
    return NextResponse.json(
