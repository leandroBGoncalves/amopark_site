...
1. **`.env.local`**
   ...
   - **Newsletter (Nodemailer):** `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `SMTP_FROM`, `NEWSLETTER_NOTIFY_EMAIL`. Veja `.env.example`. Sem SMTP, a inscrição é salva no banco, mas os e-mails automáticos não são enviados. Com SMTP ativo, cada **novo ofício**, **evento publicado ou editado** (enquanto publicado) ou **conquista** cadastrada no painel dispara um e-mail para todos os inscritos na newsletter.
